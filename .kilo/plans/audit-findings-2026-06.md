...
| Test files | 210 |
| Core test files | 60+ `*.test.ts` in `packages/core/src/__tests__/ |
...
| 2 | `WeakSet`-based event suppression keeps dead arrows alive | `packages/core/src/activity-events.ts` | high | Subagent audit: the directional mute set uses `WeakSet` (or equivalent retain cycle) so events emitted from discarded generator objects are **not** freed; events that *should* be emitted are dropped. |
