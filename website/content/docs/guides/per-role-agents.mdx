...
<Callout type="info">
  ...
  level. To set `agentConfig` (model, permissions, etc.), use the per-project `orchestrator` and
  `worker` blocks described below.
</Callout>
...
<Callout type="warning">
  **Agent inheritance is role-specific.** The top-level `agent` field in a project config is a **shared default for workers only**. Setting `agent: opencode` does **not** make the orchestrator use opencode — you must explicitly configure `orchestrator.agent` if you want a different agent for the orchestrator role.

Priority order for each role:

- **Worker:** `project.worker.agent` → `project.agent` → `defaults.worker.agent` → `defaults.agent`
- **Orchestrator:** `project.orchestrator.agent` → `defaults.orchestrator.agent` → `defaults.agent` (no shared fallback)
  </Callout>
...
- **Inheritance order.** Workers inherit from `project.agent` (shared), then `defaults.worker.agent`, then `defaults.agent`. Orchestrator sessions only inherit from `defaults.orchestrator.agent` and `defaults.agent` — they **never** fall back to the shared `project.agent` field. A project-level `worker` block fully replaces the default — it is not merged field-by-field, so specify every field you need.
