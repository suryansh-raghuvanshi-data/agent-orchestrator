...
<Callout type="info">
  AO reads unresolved review threads. Once you resolve a thread on GitHub, it drops out of the next
  nudge — so you can thumbs-up the ones the agent addressed and only the remaining ones make it back
  to the agent.
</Callout>
