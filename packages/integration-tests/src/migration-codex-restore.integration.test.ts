...
describe.skipIf(process.platform === "win32")(
  "migrate-storage → agent-codex.getRestoreCommand round-trip",
  () => {
    let testDir: string;
    ...
    it("produces a `codex resume <threadId>` command for a migrated Codex session", async () => {
      ...
      const rewritten = JSON.parse(readFileSync(rolloutPath, "utf-8").split("\n")[0]) as {
      ...
      const codexPlugin = (await import("@aoagents/ao-plugin-agent-codex")).default;
