...
export const manifest = {
  ...
  description: "Worker provider plugin: Kilo",
  version: "0.1.0",
};
...
function getTasksFilePath(): string {
  ...
  return join(baseDir, "kilo-tasks.json");
}
  ...
  return {
    ...
    async health(): Promise<WorkerProviderHealth> {
      ...
      if (offline) {
        return {
          ...
          error: "Kilo service is currently offline",
        };
      }
    ...
    async submitTask(taskConfig: WorkerProviderTaskConfig): Promise<WorkerProviderTaskHandle> {
      if (taskConfig.prompt.includes("fail:unavailable")) {
        throw new Error("Kilo service unavailable");
      }

      const tasks = loadTasks();
      const taskId = `kilo-task-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`;
    ...
    async getTaskOutput(handle: WorkerProviderTaskHandle): Promise<string> {
      ...
      return `Kilo task execution output for prompt: "${task.prompt}"`;
    },
