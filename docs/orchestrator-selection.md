...
- **Typography**: _Schibsted Grotesk_ for UI/Chrome (product voice). _JetBrains Mono_ for machine data, IDs, and terminal output. Tabular numerals for all metrics.
...
- **Elegance**: A UI that developers _want_ to keep open on a secondary monitor all day.
...
- **UI States**:
  - _Loading_: Skeleton loaders for text, subtle pulsing for cards.
  - _Empty_: Beautiful, action-oriented empty states ("Your fleet is idle. Spawn a task to begin.")
  - _Error_: Contextual error boundaries that allow retrying specific widgets without crashing the app.
